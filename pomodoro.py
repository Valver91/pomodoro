...
class PomodoroTimer:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.geometry('500x270+0+0')
        ...
        self.break_corto = ttk.Label(self.tab2, text='05:00', font=('Ubuntu', 48))
        self.break_corto.pack(pady=20)

        self.break_largo = ttk.Label(self.tab3, text='15:00', font=('Ubuntu', 48))
        self.break_largo.pack(pady=20)

        self.tabs.add(self.tab1, text='Pomodoro')
        self.tabs.add(self.tab2, text='Break Corto')
        self.tabs.add(self.tab3, text='Break Largo')
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            while full_seconds > 0 and not self.stopped:
                ...
                self.pomodoro_timer_label.config(text=f'{minutes:02}:{seconds:02}')
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            ...
            while full_seconds > 0 and not self.stopped:
                minutes, seconds = divmod(full_seconds, 60)
                self.break_corto.config(text=f'{minutes:02}:{seconds:02}')
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            if not self.stopped or self.skipped:
            ...
            while full_seconds > 0 and not self.stopped:
                minutes, seconds = divmod(full_seconds, 60)
                self.break_largo.config(text=f'{minutes:02}:{seconds:02}')
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            if not self.stopped or self.skipped:
    ...
    def reset_clock(self):
        ...
        self.break_corto.config(text='05:00')
        self.break_largo.config(text='15:00')
    ...
    def skip_clock(self):
        ...
        if current_tab == 0:
            self.pomodoro_counter_label.config(text='25:00')
        elif current_tab == 1:
            self.break_corto.config(text='05:00')
        elif current_tab == 2:
            self.break_largo.config(text='15:00')
